...
- **AI-Driven Interview Flow**: Six tailored questions (2 Easy, 2 Medium, 2 Hard) for full‑stack roles with per‑question timers and auto‑submit.
- **Progress & Recovery**: Real‑time feedback, autosave via redux‑persist/IndexedDB, and session resume.
- **Interviewer Dashboard**: Leaderboard sorted by score, search/sort, and detailed profiles with transcript, answers, scores, and AI summary.
- **Reliability & Error Handling**: Friendly validation messages, graceful fallbacks when AI is not configured, and robust parsing for DOC/DOCX/PDF.
- **Modern UI/UX**: Clean, responsive UI with shadcn‑style components, Radix UI, and subtle animations.
...
Crisp is a next‑generation AI‑powered interview assistant designed to help candidates practice effectively and give interviewers actionable insights. It delivers a real‑world experience with modern UI, robust persistence, and advanced AI features.
...
# Base URL (used for server-side fetches and metadata)
# Set one of the following in production:
NEXT_PUBLIC_SITE_URL=https://your-domain.com
# or
NEXT_PUBLIC_BASE_URL=https://your-domain.com
# Fallbacks used in server envs:
# SITE_URL and VERCEL_URL are auto-resolved when present
```
...
Ensure your `.env.local` is configured before starting to enable AI and persistence.
...
- Update `package.json` fields like `name`, `author`, and `license`.
- Edit titles and branding in `app/layout.tsx` and header components.
- Replace logos in `public/logo.png` and social images in `public/window.svg`.
- Update `LICENSE` with your name and the current year (MIT by default).

## Assessment Highlights

- All core requirements: resume upload, field prompting, AI question generation, timed interview, dashboard, persistence, pause/resume, and error handling.
- Extra polish: clean UI, subtle animations, instant feedback, robust session recovery.
- Real‑world ready: reliability, scalability, and delightful UX.

## Why Crisp Stands Out

- User‑centric: optimized for clarity, speed, and ease of use.
- AI‑first: smart question generation, evaluation, and summaries.
- Persistence: never lose progress—refresh, close, and resume anytime.
- Modern UI: product‑grade visuals and interaction design.
...
On Vercel, set `GEMINI_API_KEY`, `MONGODB_URI`, `MONGODB_DB`, and `NEXT_PUBLIC_SITE_URL` in project settings.
